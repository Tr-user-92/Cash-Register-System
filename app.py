...
#     total_spent=sum(t["base_amount"] for t in history if t["type"].lower()=="withdraw")
    
#     # Find highest spending (largest withdraw)
#     withdraws=[t for t in history if t["type"].lower()=="withdraw"]
    
#     if withdraws:
#         lowest_spending=min(withdraws,key=lambda x: x["base_amount"])
#         highest_spending=max(withdraws,key=lambda x: x["base_amount"])
#     else:
...
#         total_spending=sum(t["base_amount"] for t in monthly_transaction if t["type"].lower()=="withdraw")
...
def update_balance(phone):
    ...
    if request.method=="POST":
        ...
        # ---------- DEDUCT BALANCE ----------   
            ...
            applied_gst=amount*0.18
            final_amount=amount+applied_gst
            if user["balance"]<final_amount:
                return "No sufficient Balance in your account"
            user["balance"]-=final_amount
            transaction_type="withdraw"
